...
2. user 레코드 생성
   ...
   ```python
   # orm
   User.objects.create(first_name='길동', last_name='홍', age=600, country='Jejudo', phone='010-1234-5678', balance='9418283910293')
   ```

   ```sql
   -- sql
   INSERT INTO users_user VALUES (1001, '길동', '홍', 600, 'Jejudo', '010-1234-5678', '9418283910293');
   ```
...
4. 성, 이름 내림차순 순으로 5번째 있는 사람
   ...
   ```python
   # orm
   User.objects.order_by('-last_name', '-first_name').values('first_name')[4]
   ```
```
...
각 지역별 최고 계좌 잔액 조회
```sql
SELECT country, MAX(balance) AS max_balance FROM users_user GROUP BY country;
```
